...
const LevelEditor = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900 text-white p-4">
      <h1 className="text-2xl font-bold mb-4 text-center">Top-Down Level Editor</h1>
          ...
          {tool === 'pen' && (
            <div className="bg-gray-700 p-2 rounded text-xs">
              {currentPolygon ? (
                ...
                <p className="text-gray-400">Click to start tracing outline</p>
              )}
            </div>
          )}
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-950 rounded-lg overflow-auto">
            <div
              ...
              style={{ 
                ...
                cursor: tool === 'pen' ? 'crosshair' : tool === 'wall' ? 'crosshair' : 'default'
              }}
                    ...
                    <g key={polygon.id}>
                      ...
                      {polygon.points.map((point, i) => (
                        <circle
                          ...
                          cy={point.y}
                          r={4}
                          fill={selectedPolygonId === polygon.id ? '#ffff00' : '#00ff00'}
                        />
                      ))}
                    </g>
                  ))}
                    ...
                    <g>
                      ...
                      {currentPolygon.points.length >= 3 && 
                       distance(mousePos.x, mousePos.y, currentPolygon.points[0].x, currentPolygon.points[0].y) < CLOSE_POLYGON_DISTANCE && (
                        <circle
                          cx={currentPolygon.points[0].x}
                          cy={currentPolygon.points[0].y}
                          r={12}
                          fill="none"
                          stroke="#ffffff"
                          strokeWidth={2}
          ...
          {selectedBlock ? (
            <>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setBlocks(blocks.map(b => 
                    b.id === selectedBlockId ? { ...b, x: Number(e.target.value) } : b
                  ))}
                  className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setBlocks(blocks.map(b => 
                    b.id === selectedBlockId ? { ...b, y: Number(e.target.value) } : b
                  ))}
                  className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setBlocks(blocks.map(b => 
                    b.id === selectedBlockId ? { ...b, width: Math.max(10, Number(e.target.value)) } : b
                  ))}
                  className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setBlocks(blocks.map(b => 
                    b.id === selectedBlockId ? { ...b, height: Math.max(10, Number(e.target.value)) } : b
                  ))}
                  className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setBlocks(blocks.map(b => 
                    b.id === selectedBlockId ? { ...b, rotation: Number(e.target.value) } : b
                  ))}
                  className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                />
              </div>
              ...
              <button
                ...
                className="px-3 py-2 bg-red-600 hover:bg-red-700 rounded text-sm w-full mt-2"
              >
                Delete Block
              </button>
            </>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-gray-400">X1</label>
                  <input
                    type="number"
                    value={Math.round(selectedWall.x1)}
                    onChange={(e) => setWalls(walls.map(w => 
                      w.id === selectedWallId ? { ...w, x1: Number(e.target.value) } : w
                    ))}
                    className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                  />
                </div>
                <div>
                  <label className="block text-xs text-gray-400">Y1</label>
                  <input
                    type="number"
                    value={Math.round(selectedWall.y1)}
                    onChange={(e) => setWalls(walls.map(w => 
                      w.id === selectedWallId ? { ...w, y1: Number(e.target.value) } : w
                    ))}
                    className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                  />
                </div>
                <div>
                  <label className="block text-xs text-gray-400">X2</label>
                  <input
                    type="number"
                    value={Math.round(selectedWall.x2)}
                    onChange={(e) => setWalls(walls.map(w => 
                      w.id === selectedWallId ? { ...w, x2: Number(e.target.value) } : w
                    ))}
                    className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                  />
                </div>
                <div>
                  <label className="block text-xs text-gray-400">Y2</label>
                  <input
                    type="number"
                    value={Math.round(selectedWall.y2)}
                    onChange={(e) => setWalls(walls.map(w => 
                      w.id === selectedWallId ? { ...w, y2: Number(e.target.value) } : w
                    ))}
                    className="bg-gray-700 px-2 py-1 rounded text-sm w-full"
                  />
                </div>
            ...
            <>
              ...
              <div className="bg-gray-700 p-2 rounded">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    type="checkbox"
                    checked={selectedPolygon.inverted}
                    onChange={togglePolygonInvert}
                    className="w-4 h-4"
                  />
                  <span>Invert (Block Outside)</span>
                </label>
                <p className="text-xs text-gray-400 mt-2">
                  {selectedPolygon.inverted 
                    ? "Player is blocked OUTSIDE the traced area"
                    : "Polygon edges act as walls only"}
                </p>
              </div>
            ...
            <div className="text-gray-500 text-sm">
              ...
              <div className="mt-4 text-xs space-y-2">
                <p><strong>◼ Block:</strong> For objects/props</p>
                <p><strong>╱ Wall:</strong> Point-to-point lines</p>
                <p><strong>✎ Pen:</strong> Trace map outline, then invert to block outside</p>
              </div>
            </div>
          )}
